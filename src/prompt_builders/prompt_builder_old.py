...
# Fractions of the total prompt budget allocated per source.
# "user" is always fully included; we just account for it separately.
DEFAULT_SOURCE_BUDGETS = {
    "agent": 0.4,      # seed + agent completions
    "account": 0.4,    # account history / relevant completions
...
def estimate_tokens_from_text(text: str) -> int:
    ...
    Uses a simple 4 characters ≈ 1 token heuristic. This is intentionally
    lightweight and only used for budgeting, not billing.
    """
    if not text:
        return 0
...
def get_prompt_budget_for_agent(agent: dict) -> dict:
    """Derive a token budget for this agent.

    Supports optional fields on the agent config:
      - prompt_budget_tokens: int
      - source_budgets: {"agent": float, "account": float, "context": float}

    Falls back to DEFAULT_PROMPT_BUDGET_TOKENS and DEFAULT_SOURCE_BUDGETS
    if not provided.
    """
    prompt_budget_tokens = int(
        agent.get("prompt_budget_tokens", DEFAULT_PROMPT_BUDGET_TOKENS)
    )
    ...
    source_budgets = {
        name: int(prompt_budget_tokens * frac)
        for name, frac in normalized_fracs.items()
    }
...
class PromptBuilder(PromptBuilderInterface):
    ...
    def build_prompt(self, content_text:str, conversationId:str, agent_name, account_name, context_type="none", max_prompt_chars=6000, max_prompt_conversations=20, context_name=''):
        ...
        # --- Apply simple token-based caps per source ---

        # 1. Agent messages (seed + agent completions)
        agent_cap = source_budgets.get("agent", 0)
        if agent_cap > 0 and matched_messages_agent:
            capped_agent_messages = []
            used = 0
            for msg in matched_messages_agent:
                t = estimate_tokens_from_text(msg.content)
                if used + t > agent_cap:
                    break
                capped_agent_messages.append(msg)
                used += t
            matched_messages_agent = capped_agent_messages

        # 2. Account messages (history / relevant completions)
        account_cap = source_budgets.get("account", 0)
        if account_cap > 0 and matched_messages_account:
            capped_account_messages = []
            used = 0
            for msg in matched_messages_account:
                t = estimate_tokens_from_text(msg.content)
                if used + t > account_cap:
                    break
                capped_account_messages.append(msg)
                used += t
            matched_messages_account = capped_account_messages

        # 3. Context text (explicit context)
        context_cap = source_budgets.get("context", 0)
        if context_cap > 0 and context_text:
            context_tokens = estimate_tokens_from_text(context_text)
            if context_tokens > context_cap:
                # naive truncation by chars; 4 chars ≈ 1 token
                max_chars = context_cap * 4
