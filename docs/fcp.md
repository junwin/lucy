...
What FunctionCallingProcessor does (high level)
    ...
    A Dialogue-to-Tool bridge: it builds prompts for the LLM, lets the LLM decide to call tools, executes those tool calls via a HandlerRegistry, and optionally runs task lists (plan_tasks) produced by tools.
...
Public API: process_message(…)
    ...
    Behavior overview:
        ...
        Run the LLM loop via _run_llm_loop(…) which:
            ...
            If a plan_tasks tool is invoked and a secondary_agent + processor_factory are available, may trigger task list execution via _execute_simple_tasklist(…)
    ...
    Important behaviors and limits:
        ...
        Delegation depth and tasklists: If a tasklist (plan_tasks) is produced, the processor can execute it directly or via the supplied processor_factory/secondary_agent, depending on your setup.
...
What happens inside the main flow (step-by-step)
    ...
    _execute_tool_calls(…)
        For each tool_call:
            ...
            After getting tool_result_text, if the call is plan_tasks and there is a secondary_agent and processor_factory:
