"""Tests for Mistral adapter image content-part normalization (Step 6).

Covers:
  - _normalize_content_parts() — maps intermediate format → Mistral image_url
  - _normalize_messages() — applies normalization across all messages
  - _normalize_input_to_messages() — Case 2 normalizes content-part arrays
  - create_response() — end-to-end normalization in the API call path
  - Text parts pass through unchanged
  - Non-list content passes through unchanged
  - Mistral format: {"type": "image_url", "image_url": {"url": "data:..."}}
"""

from __future__ import annotations

from unittest.mock import Mock

import pytest

from src.llm.mistral_api import MistralApi


# ---------------------------------------------------------------------------
# Tests: _normalize_content_parts
# ---------------------------------------------------------------------------


def test_normalize_image_part_to_image_url():
    content = [
        {"type": "text", "text": "What's in this image?"},
        {"type": "image", "source": {"data": "aGVsbG8=", "mime_type": "image/png"}},
    ]

    result = MistralApi._normalize_content_parts(content)

    assert len(result) == 2
    # Text part passes through
    assert result[0] == {"type": "text", "text": "What's in this image?"}
    # Image part mapped to image_url with nested url object
    assert result[1]["type"] == "image_url"
    assert result[1]["image_url"] == {"url": "data:image/png;base64,aGVsbG8="}


def test_normalize_image_part_jpeg():
    content = [
        {"type": "image", "source": {"data": "AAAA", "mime_type": "image/jpeg"}},
    ]

    result = MistralApi._normalize_content_parts(content)

    assert result[0]["type"] == "image_url"
    assert result[0]["image_url"] == {"url": "data:image/jpeg;base64,AAAA"}


def test_normalize_image_part_gif():
    content = [
        {"type": "image", "source": {"data": "R0lG", "mime_type": "image/gif"}},
    ]

    result = MistralApi._normalize_content_parts(content)
    assert result[0]["image_url"] == {"url": "data:image/gif;base64,R0lG"}


def test_normalize_image_part_webp():
    content = [
        {"type": "image", "source": {"data": "UklG", "mime_type": "image/webp"}},
    ]

    result = MistralApi._normalize_content_parts(content)
    assert result[0]["image_url"] == {"url": "data:image/webp;base64,UklG"}


def test_normalize_missing_source_defaults_png():
    """When source is missing or not a dict, defaults to image/png."""
    content = [
        {"type": "image", "source": "not-a-dict"},
    ]

    result = MistralApi._normalize_content_parts(content)
    assert "image/png" in result[0]["image_url"]["url"]


def test_normalize_empty_source_uses_defaults():
    content = [
        {"type": "image", "source": {}},
    ]

    result = MistralApi._normalize_content_parts(content)
    assert result[0]["image_url"] == {"url": "data:image/png;base64,"}


def test_normalize_text_only_passes_through():
    content = [
        {"type": "text", "text": "Hello"},
        {"type": "text", "text": "World"},
    ]

    result = MistralApi._normalize_content_parts(content)
    assert result == content


def test_normalize_non_list_passes_through_string():
    """String content passes through unchanged."""
    result = MistralApi._normalize_content_parts("plain string")
    assert result == "plain string"


def test_normalize_non_list_passes_through_none():
    result = MistralApi._normalize_content_parts(None)
    assert result is None


def test_normalize_non_dict_part_passes_through():
    content = [
        {"type": "text", "text": "Hello"},
        "bare string in list",
    ]

    result = MistralApi._normalize_content_parts(content)
    assert result[0] == {"type": "text", "text": "Hello"}
    assert result[1] == "bare string in list"


def test_normalize_unknown_type_passes_through():
    content = [
        {"type": "custom_thing", "value": 42},
    ]

    result = MistralApi._normalize_content_parts(content)
    assert result == content


def test_normalize_multiple_images():
    content = [
        {"type": "text", "text": "Compare these:"},
        {"type": "image", "source": {"data": "aW1nMQ==", "mime_type": "image/png"}},
        {"type": "image", "source": {"data": "aW1nMg==", "mime_type": "image/jpeg"}},
    ]

    result = MistralApi._normalize_content_parts(content)

    assert len(result) == 3
    assert result[0]["type"] == "text"
    assert result[1]["type"] == "image_url"
    assert result[1]["image_url"] == {"url": "data:image/png;base64,aW1nMQ=="}
    assert result[2]["type"] == "image_url"
    assert result[2]["image_url"] == {"url": "data:image/jpeg;base64,aW1nMg=="}


# ---------------------------------------------------------------------------
# Tests: _normalize_messages
# ---------------------------------------------------------------------------


def test_normalize_messages_with_content_parts():
    messages = [
        {"role": "system", "content": "You are helpful."},
        {"role": "user", "content": [
            {"type": "text", "text": "What's this?"},
            {"type": "image", "source": {"data": "AAAA", "mime_type": "image/png"}},
        ]},
    ]

    result = MistralApi._normalize_messages(messages)

    # System message unchanged
    assert result[0] == {"role": "system", "content": "You are helpful."}
    # User message content normalized
    assert isinstance(result[1]["content"], list)
    assert result[1]["content"][0]["type"] == "text"
    assert result[1]["content"][1]["type"] == "image_url"


def test_normalize_messages_all_string_content_unchanged():
    messages = [
        {"role": "system", "content": "System"},
        {"role": "user", "content": "Hello"},
        {"role": "assistant", "content": "Hi"},
    ]

    result = MistralApi._normalize_messages(messages)
    assert result == messages


def test_normalize_messages_with_mixed_content_types():
    """Messages with both string content and list content."""
    messages = [
        {"role": "user", "content": "Plain text"},
        {"role": "user", "content": [
            {"type": "image", "source": {"data": "AAAA", "mime_type": "image/png"}},
        ]},
    ]

    result = MistralApi._normalize_messages(messages)

    assert result[0]["content"] == "Plain text"
    assert result[1]["content"][0]["type"] == "image_url"


def test_normalize_messages_non_dict_passes_through():
    messages = [
        {"role": "user", "content": "Hello"},
        "not a dict",
    ]

    result = MistralApi._normalize_messages(messages)
    assert result[0] == {"role": "user", "content": "Hello"}
    assert result[1] == "not a dict"


def test_normalize_messages_non_list_passes_through():
    result = MistralApi._normalize_messages("not a list")
    assert result == "not a list"


def test_normalize_messages_empty_list():
    result = MistralApi._normalize_messages([])
    assert result == []


def test_normalize_messages_preserves_other_message_fields():
    """Non-content fields are preserved."""
    messages = [
        {"role": "user", "content": "Hello", "name": "john", "metadata": {"key": "val"}},
    ]

    result = MistralApi._normalize_messages(messages)
    assert result[0]["role"] == "user"
    assert result[0]["content"] == "Hello"
    assert result[0]["name"] == "john"
    assert result[0]["metadata"] == {"key": "val"}


# ---------------------------------------------------------------------------
# Tests: _normalize_input_to_messages (Case 2 — content-part arrays)
# ---------------------------------------------------------------------------


def test_normalize_input_to_messages_with_image_parts():
    """_normalize_input_to_messages Case 2 normalizes image content parts."""
    api = MistralApi(client=Mock())

    messages = [
        {"role": "user", "content": [
            {"type": "text", "text": "What is this?"},
            {"type": "image", "source": {"data": "AAAA", "mime_type": "image/png"}},
        ]},
    ]

    result = api._normalize_input_to_messages(messages)

    assert len(result) == 1
    assert result[0]["role"] == "user"
    assert result[0]["content"][0] == {"type": "text", "text": "What is this?"}
    assert result[0]["content"][1] == {
        "type": "image_url",
        "image_url": {"url": "data:image/png;base64,AAAA"},
    }


def test_normalize_input_to_messages_pure_text_passes_through():
    """_normalize_input_to_messages leaves text-only messages unchanged."""
    api = MistralApi(client=Mock())

    messages = [
        {"role": "system", "content": "You are helpful."},
        {"role": "user", "content": "Hello world"},
    ]

    result = api._normalize_input_to_messages(messages)
    assert result == messages


def test_normalize_input_to_messages_handles_multiple_messages_with_images():
    """Each message with list content is normalized independently."""
    api = MistralApi(client=Mock())

    messages = [
        {"role": "user", "content": [
            {"type": "image", "source": {"data": "aW1nMQ==", "mime_type": "image/png"}},
        ]},
        {"role": "user", "content": [
            {"type": "image", "source": {"data": "aW1nMg==", "mime_type": "image/jpeg"}},
        ]},
    ]

    result = api._normalize_input_to_messages(messages)

    assert result[0]["content"][0]["type"] == "image_url"
    assert "aW1nMQ==" in result[0]["content"][0]["image_url"]["url"]
    assert result[1]["content"][0]["type"] == "image_url"
    assert "aW1nMg==" in result[1]["content"][0]["image_url"]["url"]


def test_normalize_input_to_messages_string_input_unchanged():
    """Non-list input falls through Case 3."""
    api = MistralApi(client=Mock())

    # string → not a list, not a dict → falls to Case 4 (empty list)
    # Actually, Case 2: not list. Case 3: not dict. Case 4: returns []
    result = api._normalize_input_to_messages("just a string")
    assert result == []


def test_normalize_input_to_messages_dict_input_wrapped():
    """Single dict is wrapped in a list (Case 3)."""
    api = MistralApi(client=Mock())

    result = api._normalize_input_to_messages({"role": "user", "content": "hi"})
    assert result == [{"role": "user", "content": "hi"}]


def test_normalize_input_to_messages_preserves_original_input():
    """_normalize_input_to_messages does not mutate the caller's list."""
    api = MistralApi(client=Mock())

    messages = [
        {"role": "user", "content": [
            {"type": "image", "source": {"data": "AAAA", "mime_type": "image/png"}},
        ]},
    ]

    original = [
        {"role": "user", "content": [
            {"type": "image", "source": {"data": "AAAA", "mime_type": "image/png"}},
        ]},
    ]

    api._normalize_input_to_messages(messages)
    assert messages == original


# ---------------------------------------------------------------------------
# Tests: create_response integration
# ---------------------------------------------------------------------------


def _make_mock_mistral_client():
    """Build a mock OpenAI client whose chat.completions.create returns a fake response."""
    client = Mock()

    fake_choice = Mock()
    fake_choice.message.content = "I see a cat in the image."
    fake_choice.message.tool_calls = None

    fake_resp = Mock()
    fake_resp.id = "mistral-resp-abc-123"
    fake_resp.model = "pixtral-large-latest"
    fake_resp.choices = [fake_choice]
    fake_resp.usage = Mock()
    fake_resp.usage.prompt_tokens = 100
    fake_resp.usage.completion_tokens = 50
    fake_resp.usage.total_tokens = 150

    client.chat.completions.create.return_value = fake_resp
    return client


def test_create_response_normalizes_image_parts():
    """create_response normalizes intermediate image format before calling Mistral."""
    client = _make_mock_mistral_client()
    api = MistralApi(client=client)

    messages = [
        {"role": "system", "content": "You are vision-capable."},
        {"role": "user", "content": [
            {"type": "text", "text": "Describe this:"},
            {"type": "image", "source": {"data": "YWJjZA==", "mime_type": "image/jpeg"}},
        ]},
    ]

    api.create_response(model="pixtral-large-latest", input=messages)

    # Verify client was called with normalized input
    call_kwargs = client.chat.completions.create.call_args.kwargs
    actual_messages = call_kwargs["messages"]

    assert actual_messages[0] == messages[0]  # system message unchanged
    assert actual_messages[1]["content"][0] == {"type": "text", "text": "Describe this:"}
    assert actual_messages[1]["content"][1] == {
        "type": "image_url",
        "image_url": {"url": "data:image/jpeg;base64,YWJjZA=="},
    }


def test_create_response_passes_through_plain_text():
    """create_response leaves plain text messages unchanged."""
    client = _make_mock_mistral_client()
    api = MistralApi(client=client)

    messages = [
        {"role": "system", "content": "You are helpful."},
        {"role": "user", "content": "Hello world"},
    ]

    api.create_response(model="pixtral-large-latest", input=messages)

    call_kwargs = client.chat.completions.create.call_args.kwargs
    assert call_kwargs["messages"] == messages


def test_create_response_preserves_original_messages():
    """create_response does not mutate the caller's input list."""
    client = _make_mock_mistral_client()
    api = MistralApi(client=client)

    messages = [
        {"role": "user", "content": [
            {"type": "image", "source": {"data": "AAAA", "mime_type": "image/png"}},
        ]},
    ]

    original = [
        {"role": "user", "content": [
            {"type": "image", "source": {"data": "AAAA", "mime_type": "image/png"}},
        ]},
    ]

    api.create_response(model="pixtral-large-latest", input=messages)
    assert messages == original
