"""Tests for the Galet prompt adapter's attachment resolver."""
...
def _make_attachment_resolver(images_root, allowed_tools=None):
    del allowed_tools
    config = Mock()
    ...
    return AttachmentResolver(config)


@pytest.fixture
def pb_with_temp_dir(temp_images_root):
    return _make_attachment_resolver(temp_images_root)
...
class TestGuessMimeFromPath:
    def test_png(self, pb_with_temp_dir):
        assert pb_with_temp_dir.guess_mime_from_path("screenshot.png") == "image/png"

    def test_jpg(self, pb_with_temp_dir):
        assert pb_with_temp_dir.guess_mime_from_path("photo.jpg") == "image/jpeg"

    def test_jpeg(self, pb_with_temp_dir):
        assert pb_with_temp_dir.guess_mime_from_path("photo.jpeg") == "image/jpeg"

    def test_gif(self, pb_with_temp_dir):
        assert pb_with_temp_dir.guess_mime_from_path("animation.gif") == "image/gif"

    def test_webp(self, pb_with_temp_dir):
        assert pb_with_temp_dir.guess_mime_from_path("image.webp") == "image/webp"

    def test_unknown_extension(self, pb_with_temp_dir):
        assert pb_with_temp_dir.guess_mime_from_path("data.bin") == "application/octet-stream"

    def test_no_extension(self, pb_with_temp_dir):
        assert pb_with_temp_dir.guess_mime_from_path("README") == "application/octet-stream"
...
class TestFindImageFile:
    def test_finds_matching_file(self, temp_images_root, pb_with_temp_dir):
        ...
        assert pb_with_temp_dir.find_image_file(
    ...
    def test_returns_none_when_nonexistent(self, temp_images_root, pb_with_temp_dir):
        ...
        assert pb_with_temp_dir.find_image_file(images_dir, "testuser", "nonexistent123") is None
...
class TestResolveAttachments:
    def test_image_ids_resolved_to_base64(self, temp_images_root):
        ...
        parts = _make_attachment_resolver(temp_images_root).resolve(
...
class TestResolveMixedAttachments:
    ...
    def test_empty_ids_produces_empty_list(self, temp_images_root):
        assert _make_attachment_resolver(temp_images_root).resolve(
    ...
    def test_png_with_uppercase_extension(self, temp_images_root):
        ...
        assert _make_attachment_resolver(temp_images_root).guess_mime_from_path(path) == "image/png"
...
class TestSupportsImages:
    def test_supports_images_false_emits_markers_with_instruction(self, temp_images_root):
        ...
        parts = _make_attachment_resolver(temp_images_root, allowed_tools=["file_load", "file_save"]).resolve(
            account_name="junwin",
            image_ids=["imgdel1"],
            file_ids=None,
    ...
    def test_supports_images_false_emits_instruction_regardless_of_tools(self, temp_images_root):
        ...
        parts = _make_attachment_resolver(temp_images_root, allowed_tools=[]).resolve(
    ...
    def test_supports_images_false_multiple_images(self, temp_images_root):
        ...
        parts = _make_attachment_resolver(temp_images_root, allowed_tools=["tasklists_manage"]).resolve(
            account_name="junwin", image_ids=["img001", "img002"], file_ids=None,
